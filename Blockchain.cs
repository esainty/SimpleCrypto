...
public class Blockchain {
    ...
    public Blockchain() {
        string genesisHash = generateBlockHash(0, "", 0, "Genesis");
        blockchain.Add(new Block(0, "0", genesisHash, 0, "Genesis"));
    }

    public bool replaceChain(List<Block> newChain) {
        if (isValidChain(newChain) && newChain.Count > blockchain.Count) {
            blockchain = newChain;
            return true; 
        } else {
     ...
     private Block generateBlock(string blockData) {
        Block previousBlock = latestBlock;
        int nextIndex = previousBlock.index + 1;
        int nextTimestamp = _timestamp;
        string nextHash = generateBlockHash(nextIndex, previousBlock.hash, nextTimestamp, blockData);
        return new Block(nextIndex, previousBlock.hash, nextHash, nextTimestamp, blockData);
    }

    private string generateBlockHash(int index, string previousHash, int timestamp, string data) {
    ...
    private bool isValidBlock(Block newBlock, Block previousBlock) {
        ...
        if (generateBlockHash(newBlock.index, newBlock.previousHash, newBlock.timestamp, newBlock.data) != newBlock.hash) {
